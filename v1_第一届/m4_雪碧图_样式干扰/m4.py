...
def get_base64_data(page):
    ...
    cookies = {
        "sessionid": "n9re843358ud2ynr1o0d6lfk91y9g37o",
    }
    ...
    value = rjs['value']
    info = rjs['info']
    # 8cfc39c05284c7e244dafa73b0577dd9
    # fed51dd99f2233ac7091e128fc30b44e
...
def get_j_key(base64_data):
    ...
    如果 hex_md5 一样就不被展示 不一样就被展示
    :return:
    """

    base64_key = base64.b64encode(str(base64_data['key'] + base64_data['value']).encode()).decode()
    j_key = hex_md5(base64_key.replace("=", ""))
...
def get_data(page):
    base64_data = get_base64_data(page)
    j_key = get_j_key(base64_data)
    info = base64_data['info']
    # 解析每一个td
    rule = re.compile(r'<td>(.*?)</td>')
    td_list = rule.findall(info)

    for td in td_list:
        #  获取 图片j_key的列表 ，图片源代码的列表 以及偏移量的列表
        img_number_list = re.findall(r'class="img_number (\w+)"', td)
        # base64_img_list = re.findall(r'src="(.*?)"', info)
        ...
        for index, (img_number, base64_img, left_offset_number) in enumerate(
                zip(img_number_list, base64_img_list, left_offset_number_list)):
            # 如果 j_key 等于提取到的class里的那个值 就跳过因为这个是赋予 display None 的不现实
            if img_number == j_key:
                continue
            # 拿到 解析后的数据 用索引去提取
            display_img_base64_list.append(base64_img_list[index])

            num_list.append(base64_img_relation[base64_img])
